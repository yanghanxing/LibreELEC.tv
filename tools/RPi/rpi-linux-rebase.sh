...
# Setup:
# git clone -b rpi-4.6.y --depth=1000 --single-branch git://github.com/raspberrypi/linux.git raspberrypi-linux
# git remote add -t linux-4.6.y linux-stable git://git.kernel.org/pub/scm/linux/kernel/git/stable/linux-stable.git
# git config --local core.abbrev 12
# Update:
# git fetch --all --depth=1000
# git reset --hard origin/rpi-4.6.y
# GIT_SEQUENCE_EDITOR=../rpi-linux-rebase-4.6.sh git rebase -i linux-stable/linux-4.6.y
# git format-patch --no-renames --no-signature --stdout linux-stable/linux-4.6.y > ../linux-01-RPi_support-4.6.patch

TODO=$1

# Drop commits already in linux-stable/linux-4.6.y
DROP_COMMITS=""
...
DROP_COMMITS="net: Add non-mainline source for rtl8192cu wlan
